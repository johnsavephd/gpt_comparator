...
def main():
  with st.sidebar:  
      ...
      with st.form(key='gpt_parameters', border = False):
        ...
        context = st.text_area(label = "🤖 **Role of the system**", key = "context", value="You are an helpful assistant", max_chars=100, placeholder= "You are an helpful assistant", help = "Set the role for the assistant. If you want, specify also the personality (ex: irriverent, funny).")
        prompt = st.text_area(label = ":question: **Your Prompt**", key = "prompt", value="", max_chars=250, placeholder= "Insert here your prompt", help = "Type here an instruction in natural language." )
  ...
  with tabMain:
    ...
    with col3:
      ...
      with tabInfo:
        ...
        st.markdown("Have you ever wondered what the main differences are among OpenAI models? Choosing the right model can be a bit overwhelming. Use this application to compare the results from different models and see the differences for yourself. Yes it's fun, but make sure to check out the documentation afterward! 😊")
